...
- (void)uploadImage:(NSString *)imageUrl {
    NSString *funcName = [NSString stringWithFormat:@"uploadImage('%@');", imageUrl];
    [_webView stringByEvaluatingJavaScriptFromString:funcName];
}

- (void)uploadVideo:(NSString *)videoUrl {
    NSString *funcName = [NSString stringWithFormat:@"uploadVideo('%@');", videoUrl];
    [_webView stringByEvaluatingJavaScriptFromString:funcName];
}
